...
```text
proxmox_virtual_environment_vm.k3s["k3s-master-1"] -> VMID 300
```
...
`local.inventory_masters` использует `proxmox_virtual_environment_vm.k3s`, значит inventory создаётся после того, как Terraform узнает IP VM.
